...
function render_html(cannolo, options, callback) {
  ...
  md_conrefs.init(cannolo.files, {blockPrefixChar: "\\*", blockPrefixCharOptional: true});
  ...
  if (options.split) {
    ...
    for (var key in list) {
      if (list.hasOwnProperty(key) && list[key].type === "class" && list[key].superclass === undefined) {
        ...
        vars.tree = { };
        vars.tree.children = [cannolo.tree.children[keyMapping[key]]];
